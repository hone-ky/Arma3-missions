if (isserver) then {
    ...
    if (_eastUnitsInTrigger <= 70) then {
        _rNumberGroups = floor (random 5) + 1;
        for "_j" from 1 to _rNumberGroups do {
            _selectedConfigIndex = floor (random (count _groupConfigs));
            _selectedConfig = _groupConfigs select _selectedConfigIndex;
            _group = [_triggerPosATL, independent, _selectedConfig] call BIS_fnc_spawnGroup;
        ...
        _rNumberHilux = floor (random 5) + 1;
