...
// 3. トレーニング (Virolahti)
private _spawnTrainingAction = [
    "spawn_training_virolahti",
    "トレーニング開始 (Virolahti)",
    "\a3\ui_f\data\igui\cfg\simpleTasks\types\target_ca.paa",
    { [ [player, player], "training_virolahti.sqf" ] remoteExec ["execVM", 2]; },
    { true }
] call ace_interact_menu_fnc_createAction;
...
private _spawnCPAction = [
    "spawn_chdkz_cp",
    "ChDKZ Checkpoint (DMR作戦)",
    "\a3\ui_f\data\igui\cfg\simpleTasks\types\target_ca.paa",
    { [ [player], "scripts\ChDKZ_CheckPoint\fn_main.sqf" ] remoteExec ["execVM", 2]; },
    { true }
] call ace_interact_menu_fnc_createAction;
...
private _cleanupCPAction = [
    ...
    { [ [], "scripts\ChDKZ_CheckPoint\fn_cleanup.sqf" ] remoteExec ["execVM", 2]; },
    { true }
] call ace_interact_menu_fnc_createAction;
...
// サブメニューを一括登録
{
    ...
    _spawnBaseAction,
    _spawnWaveAction,
    _spawnTrainingAction,
    _spawnHardTownAction,
    _spawnInsaneTownAction, // ← 元からあるここの末尾にカンマ(,)を忘れずに追加！
    _spawnCPAction,         // ★追加
    _cleanupCPAction        // ★追加 // 追加
];
